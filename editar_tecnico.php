...
// Traer la información del especialista asegurando que mantenga su rol correspondiente
$stmt = $pdo->prepare("SELECT * FROM usuarios WHERE id = ? AND rol = 'tecnico'");
$stmt->execute([$id]);
$tecnico = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($nombre) && !empty($correo)) {
        try {
            $stmt_update = $pdo->prepare("UPDATE usuarios SET nombre = ?, correo = ? WHERE id = ? AND rol = 'tecnico'");
            $stmt_update->execute([$nombre, $correo, $id]);
            
            header("Location: gestion_recursos.php?status=tecnico_updated");
            exit;
        } catch (PDOException $e) {
            if ($e->getCode() == 23000) {
                $mensaje = "Ese correo ya está registrado por otro operador.";
                $clase_alerta = "is-danger";
            } else {
                $mensaje = "Error al actualizar la ficha del especialista.";
                $clase_alerta = "is-danger";
            }
        }
    } else {
        $mensaje = "Por favor, completa los datos de perfil mínimos requeridos.";
...
<head>
    ...
    <title>Modificar Especialista - Mantenimiento IA</title>
    ...
    <main class="section main-content-area">
        <div class="container is-max-desktop" style="margin-top: 25px;">
            ...
            <div class="block mb-6" style="padding-left: 10px;">
                <h1 class="title is-2" style="letter-spacing: 2px; font-weight: 800; text-transform: uppercase;">Actualizar Técnico</h1>
                <h2 class="subtitle is-6 mt-1" style="color: #cbd5e1 !important;">
                    Ajustando credenciales para el operador: <span class="theme-accent-text" style="font-weight: 500;"><?php echo htmlspecialchars($tecnico['nombre']); ?></span>
                </h2>
            </div>
            ...
            <div class="box custom-card p-5">
                <form action="" method="POST">
                    <div class="field mb-4">
                        ...
                        <div class="control">
                            <input class="input" type="text" name="nombre" value="<?php echo htmlspecialchars($tecnico['nombre']); ?>" required>
                        </div>
                    </div>
                    ...
                    <div class="field mb-4">
                        ...
                        <div class="control">
                            <input class="input" type="email" name="correo" value="<?php echo htmlspecialchars($tecnico['correo']); ?>" required>
                        </div>
                    </div>
